...
def MassFlow_or_FunnelFlow(X1, X2, Y1, Y2, sigmav, sigma1, PHIE, rhob, WFA, UYS, sigmaf, N, number,RADIUS):
    ...
    if (WFA[-1]>PHIE[-1]):
        WFA[-1] = PHIE[-1]          #To avoid nan to estimate beta in the next line
    ...
    # angle of the outlet of the hopper from a vertical line (I added 0.000000000000001 to the denominator to avoid having the denominator equal to zero)
    theta = 90 - np.arctan((Y2 - Y1) / (X2 - X1 + 0.000000000000001)) * 180 / np.pi

    F = -1                                          # F=1 is funnel flow and F=0 is mass flow in the passive state
    P = -1                                          # If F=0, P=0 is no-arch and P=1 is equivalent to arch formation
    ...
    if (F == 0):                                    # if there is a mass flow, determine if there is a risk of arch formation
        ...
        # X and Y are just two parameters needed to estimate the major principal stress at the outlet of the hopper
        ...
        Y = ((2 * (1 - np.cos(math.radians(beta + theta)))) * np.sin(math.radians(theta)) + np.sin(
        ...
        sigma1_out = (1 + np.sin(math.radians(PHIE[-1]))) * Y * (rhob[-1]) * 9.8 * B / (
